...
export function explainProjectHealth(
  ...
  if (project.metrics.tvl !== undefined) {
    ...
    if (tvlScore > 50)
      reasons.push(
        `Strong TVL ($${(project.metrics.tvl / 1_000_000).toFixed(1)}M) contributes positively to health`,
      );
    else if (tvlScore > 10)
      reasons.push(
        `Moderate TVL ($${(project.metrics.tvl / 1_000_000).toFixed(1)}M) provides some health support`,
      );
    else
      reasons.push(
        `Low TVL ($${(project.metrics.tvl / 1_000_000).toFixed(1)}M) limits health score`,
      );
  } else {
  ...
  if (project.metrics.volume24h !== undefined && project.metrics.volume24h > 0) {
    ...
    if (volScore > 30)
      reasons.push(
        `Healthy 24h volume ($${(project.metrics.volume24h / 1_000_000).toFixed(1)}M) signals activity`,
      );
    else
      reasons.push(
        `Low 24h volume ($${(project.metrics.volume24h / 1_000_000).toFixed(1)}M) suggests limited trading activity`,
      );
  }
  ...
  if (project.metrics.developerActivity !== undefined && project.metrics.developerActivity > 0) {
    ...
    if (devScore > 40)
      reasons.push(
        `Strong developer activity (${project.metrics.developerActivity} commits) supports long-term health`,
      );
    else
      reasons.push(
        `Some developer activity (${project.metrics.developerActivity} commits) detected`,
      );
  } else {
  ...
  if (evidence.length > 0) {
    ...
    if (verifiedCount > 0)
      reasons.push(`${verifiedCount} verified evidence item(s) back this assessment`);
  ...
  if (risk > 60)
    reasons.push(
      `Elevated risk (${Math.round(risk)}/100) due to sparse/stale evidence or missing metrics`,
    );
  else if (risk > 30)
    reasons.push(`Moderate risk (${Math.round(risk)}/100) — some evidence gaps or staleness`);
  else reasons.push(`Low risk (${Math.round(risk)}/100) — good evidence coverage and freshness`);

  // Momentum
  if (momentum > 25)
    reasons.push(
      `Positive momentum (+${Math.round(momentum)}%) — evidence and developer signals trending up`,
    );
  else if (momentum < -25)
    reasons.push(
      `Negative momentum (${Math.round(momentum)}%) — declining evidence support or activity`,
    );
...
export function explainNarrative(
  ...
  if (evidence.length > 0) {
    ...
    const demoCount = evidence.filter((e) => e.status === "demo").length;
    reasons.push(
      `Backed by ${evidence.length} evidence item(s): ${verifiedCount} verified, ${demoCount} demo`,
    );
...
export function explainSignal(
  signal: {
    ...
    confidence: number;
    evidenceIds: string[];
    supportingEvidence: { evidenceId: string; relationship: string; weight?: number }[];
  },
  ...
  reasons.push(
    `${signal.title} (type: ${signal.type}, confidence: ${Math.round(signal.confidence * 100)}%)`,
  );
  ...
  if (signal.confidence >= 0.7)
    reasons.push("High confidence — multiple verified sources, strong correlation");
  else if (signal.confidence >= 0.4)
    reasons.push("Moderate confidence — some correlation, limited provider diversity");
...
export function explainReport(
  ...
  if (report.sections.thesis) {
    ...
    const relevantNarrative = narratives.find(
      (n) =>
    ...
    if (relevantNarrative) {
      reasons.push(
        `Driven by ${relevantNarrative.narrative.name} narrative (momentum: ${Math.round(relevantNarrative.momentum)}%)`,
      );
    }
  }
  ...
  if (report.sections.catalyst) {
    ...
    if (strongest) {
      reasons.push(
        `Strongest project in scope: ${strongest.project.name} (health: ${Math.round(strongest.health.health)}/100, risk: ${Math.round(strongest.health.risk)}/100)`,
      );
    }
  }
  ...
  if (report.evidenceIds.length > 0) {
    ...
    const demoCount = evidence.filter((e) => e.status === "demo").length;
    reasons.push(
      `Report backed by ${report.evidenceIds.length} evidence reference(s): ${verifiedCount} verified, ${demoCount} demo`,
    );
  }
