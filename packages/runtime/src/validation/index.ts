...
export function validateRequiredString(
  ...
  if (typeof value !== "string")
    return {
      ok: false,
      error: InsightErrors.validationError(`${fieldName} must be a string`, {
        field: fieldName,
  ...
  if (!trimmed)
    return {
      ok: false,
      error: InsightErrors.validationError(`${fieldName} is required`, { field: fieldName }),
    };
  if (options?.maxLength && trimmed.length > options.maxLength)
    return {
      ...
      error: InsightErrors.validationError(
        ...
        { field: fieldName, maxLength: options.maxLength, actualLength: trimmed.length },
      ),
    };
  if (options?.pattern && !options.pattern.test(trimmed))
    return {
      ok: false,
      error: InsightErrors.validationError(
        `${fieldName} has invalid format${options.patternName ? ` (expected ${options.patternName})` : ""}`,
...
export function validateReferenceDate(
  ...
  if (!iso8601Regex.test(dateStr) || Number.isNaN(Date.parse(dateStr)))
    return {
      ok: false,
      error: InsightErrors.validationError(
...
export function validateHistoryRange(
  ...
  if (Date.parse(fromResult.value) > Date.parse(toResult.value))
    return {
      ok: false,
      error: InsightErrors.validationError("from date must be before or equal to to date", {
...
export function validatePositiveInteger(
  ...
  if (typeof value !== "number" || !Number.isInteger(value))
    return {
      ok: false,
      error: InsightErrors.validationError(`${fieldName} must be an integer`, {
        field: fieldName,
        received: typeof value,
      }),
    };
  if (value <= 0)
    return {
      ok: false,
      error: InsightErrors.validationError(`${fieldName} must be positive`, {
        field: fieldName,
        value,
      }),
    };
  if (options?.max && value > options.max)
    return {
      ok: false,
      error: InsightErrors.validationError(`${fieldName} exceeds maximum of ${options.max}`, {
        field: fieldName,
        max: options.max,
        value,
      }),
...
export function validateJobId(value: unknown): ValidationResult<string> {
  return validateRequiredString(value, "jobId", {
    pattern: /^[a-zA-Z0-9_-]+$/,
...
export function safeJsonParse<T>(input: unknown, context?: string): ValidationResult<T> {
  if (typeof input !== "string")
    return {
      ok: false,
      error: InsightErrors.validationError("Input must be a JSON string", {
        context,
  ...
  try {
    ...
    return {
      ok: false,
      error: InsightErrors.validationError("Invalid JSON input", {
        context,
