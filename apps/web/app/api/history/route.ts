...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (fromId === null || toId === null) {
      return errorResponse("VALIDATION_ERROR", 'Both "from" and "to" query parameters are required.', 400, { received: { from: fromId, to: toId } }, requestId);
    }
    const fromResult = validateSnapshotId(fromId);
    if (!fromResult.ok) return errorResponse("VALIDATION_ERROR", fromResult.error.message, 400, fromResult.error.details, requestId);
    const toResult = validateSnapshotId(toId);
    if (!toResult.ok) return errorResponse("VALIDATION_ERROR", toResult.error.message, 400, toResult.error.details, requestId);
